...
export const DeviceInfo = ({ dongleId }: { dongleId: string }) => {
  const res = api.devices.get.useQuery([dongleId], { params: { dongleId } })
  ...
  return (
    <>
      <TopAppBar
        className="font-bold"
        leading={
            !modal ? <img alt="" src="/images/comma-white.png" className="h-8" />:<DrawerToggleButton/>
        }
      >
        connect
      </TopAppBar>
      <div className="flex flex-col gap-4 px-4 pb-4">
        <div className="h-min overflow-hidden rounded-lg bg-surface-container-low">
          ...
          <div className="flex items-center justify-between p-4">
            {!device ? <div className="h-[32px] skeleton-loader size-full rounded-xs" />:
        ...
        <div className="flex flex-col gap-2">
            ...
            {snapshot.isLoading && <div className="flex-1 overflow-hidden rounded-lg bg-surface-container-low">
              <div className="p-4">
                <div>Loading snapshots...</div>
              </div>
            </div>}
            {snapshot.isError && <div className="flex-1 overflow-hidden rounded-lg bg-surface-container-low">
