  ...
  const setupRTCConnection = async () => {
    ...
    try {
      ...
      try {
        ...
        localAudioTrack.current.enabled = false // Start muted
      } catch (e) {
        console.warn('Failed to get user media', e)
        setError('Microphone access denied or unavailable')
        // Proceed without audio if failed? Or better to let user know?
        // For now continuing but 2-way audio won't work sending side.
      }
      ...
      pc.ontrack = (event) => {
        ...
        // Ensure we add both video and audio tracks
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {streams.map((item, i) => (
            <video
              ...
              autoPlay
              playsInline
              // Removed 'muted' to hear remote audio.
              // Note: If remote sends video+audio in same stream, this works.
              // If distinct streams, they render separately.
              // We might need to ensure audio only plays from one source or handle mix.
              ref={(video) => {
