...
export const restoreFromR2 = async () => {
  ...
  if (!client) return
  if (existsSync(env.DB_PATH)) return console.log('[backup] Local database exists, skipping restore')

  console.log('[backup] No local database, checking R2...')
  const latestKey = await getLatestBackupKey(client)
  if (!latestKey) return console.log('[backup] No backup in R2 (first run)')
